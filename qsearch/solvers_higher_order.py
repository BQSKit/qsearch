...
class Objective:
    def eval_func(self, options):
        return self.error_func(options)

    def error_func(self, options):
        created_error_residuals = self.error_residuals(options)
        if created_error_func is None:
            return None
        def created_error_func(circuit, parameters, target):
            return np.sum(np.square(created_error_residuals(circuit, parameters, target)))
        return created_error_func

    def error_jac(self, options):
        created_error_residuals_jac = self.error_residuals_jac(options)
        if created_error_residuals_jac is None:
            return None
        def created_error_jac(circuit, parameters, target):
            return 2*np.sum(created_error_residuals(circuit, parameters, target))
        return created_error_jac

    def error_residuals(self, options):
        return None # you generally can create an error_func from error_residuals, but not the other way around

    def error_residuals_jac(self, options):
        return None
...
class MatrixDistanceObjective(Objective):
    ...
    def error_residuals(self, options):
        ...
        def created_error_residuals(circuit, parameters, target):
            return comparison.matrix_residuals(target, circuit.matrix(parameters), I)
        return created_error_residuals

    def error_residuals_jac(self, options):
        def created_error_residuals_jac(circuit, parameters, target):
            return comparison.matrix_residuals_jac(target, *circuit.mat_jac(parameters))
        return created_error_residuals_jac
...
class COBYLA_Solver_Objective(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ef = options.objective.error_func(options)
        target = options.target
        eval_func = lambda v: ef(target, circuit.matrix(v))
...
class BFGS_Jac_Solver_Objective(Solver):
    """A solver based on the BFGS implementation in scipy.  It requires gradients."""
    def solve_for_unitary(self, circuit, options, x0=None):
        ej = options.objective.error_jac(options)
        target = options.target
        def eval_func(v):
            M, jacs = circuit.mat_jac(v)
            return ej(target, M, jacs)
...
class LeastSquares_Jac_Solver_Objective(Solver):
    ...
    def solve_for_unitary(self, circuit, options, x0=None):
        ...
        obef = options.objective.error_residuals(options)
        obja = options.objective.error_residuals_jac(options)
        target = options.target
        eval_func = lambda v: obef(circuit, v, target)
        jac_func = lambda v: obja(circuit, v, target)
